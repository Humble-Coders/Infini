...
// TEMP — client color-scheme review. Reuses the real homepage content so the
// client is judging the palette on actual layout, not a mockup. Delete this
// whole route (and app/globals.css's `[data-theme=...]` block) once a
// direction is picked. Never linked from the live site; noindex below.
// Demo case studies kept in sync with app/(public)/page.tsx's DEMO_CASE_STUDIES.
...
export const metadata: Metadata = {
  title: "Color scheme preview | INFINI",
  robots: { index: false, follow: false },
};

export function generateStaticParams() {
  return PREVIEW_SCHEMES.map((scheme) => ({ scheme: scheme.slug }));
}

export default async function ThemePreviewPage({ params }: { params: Promise<{ scheme: string }> }) {
  const { scheme } = await params;
  if (!PREVIEW_SCHEMES.some((s) => s.slug === scheme)) notFound();
  ...
  return (
    <div data-theme={scheme as PreviewSchemeSlug}>
      <SmoothScroll>
      ...
      <ThemeSwitcher current={scheme as PreviewSchemeSlug} />
    </div>
  );
}
