...
- **Incident #2: local `.env` had a typo in `NEXT_PUBLIC_FIREBASE_API_KEY`** (`...FdYyK6Ba3...` vs the real `...FdYy6KBa3...`, a transposed character), unrelated to any code in this diff — pre-existing in the developer's local `.env` from an earlier ticket, not committed (`.env` is gitignored). Caused every client-side sign-in attempt to fail with a generic "Sign-in failed" message. Diagnosed by comparing against the project's real config (`firebase apps:sdkconfig WEB ... --project infini-2fdec`) after confirming server-side that the Auth user itself was correctly provisioned (not disabled, correct claim, `password` provider present) but had never completed a sign-in. Fixed locally; nothing to commit since `.env` isn't tracked, but flagging in case other local `.env` copies have the same typo.

## Open questions / follow-ups

- ~~`npm run test:rules` needs to actually be run~~ — still not run in this environment (Java 21+ required, only 17 available here). The file is written and type-checks, but its assertions remain unverified by execution; run it once on a machine with a newer JDK, or in CI, before merge.
- ~~Confirm sign-in works end-to-end~~ — **done.** Signed in as `nisheshsingla@gmail.com` via `/admin/sign-in` after the `.env` API-key fix above; lands on the `/admin` placeholder correctly.
- Still not exercised: the Users screen's invite flow and role-change flow (`inviteAdminUser`/`setUserRole` calls against the live functions) — code and deploy are done, but nobody has clicked through it yet.
