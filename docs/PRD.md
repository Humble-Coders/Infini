...
| **D4** | **SMTP via Google App Password** for enquiry notification | No new vendor, no ongoing cost; permitted by the brief. | Weaker deliverability to corporate inboxes, Gmail send limits, no bounce visibility, and app passwords break on Google policy changes. **Mitigation: leads are written to Firestore before email is attempted, so no enquiry can be lost to an email failure; email errors are logged and surfaced in the admin dashboard.** |
| **D5** | **Agency owns the Firebase/GCP project during build; ownership transfers to INFINI at handover** | Blaze plan is required for App Hosting, Cloud Functions and reCAPTCHA Enterprise. Agency ownership means development is never blocked on client billing setup. | Agency carries the Blaze bill during the build. Project transfer must be an explicit go-live checklist item, not an afterthought. |
